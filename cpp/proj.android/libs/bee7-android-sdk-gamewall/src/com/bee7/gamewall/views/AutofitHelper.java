...
public class AutofitHelper {
    ...
    public static AutofitHelper create(TextView view, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            ...
            TypedArray ta = context.obtainStyledAttributes(
                    ...
                    defStyle,
                    0);
            sizeToFit = ta.getBoolean(R.styleable.AutofitTextView_autoFitSizeToFit, sizeToFit);
            minTextSize = ta.getDimensionPixelSize(R.styleable.AutofitTextView_autoFitMinTextSize,
                    minTextSize);
            precision = ta.getFloat(R.styleable.AutofitTextView_autoFitPrecision, precision);
            ta.recycle();
    ...
    public AutofitHelper setEnabled(boolean enabled) {
        if (mEnabled != enabled) {
            ...
            if (enabled) {
                ...
                mTextView.addOnLayoutChangeListener(mOnLayoutChangeListener); //TODO Call requires API level 11 (current min is 9)

                autofit();
            } else {
                mTextView.removeTextChangedListener(mTextWatcher);
                mTextView.removeOnLayoutChangeListener(mOnLayoutChangeListener); //TODO Call requires API level 11 (current min is 9)
    ...
    private class AutofitOnLayoutChangeListener implements View.OnLayoutChangeListener { //TODO Call requires API level 11 (current min is 9)
        @Override
